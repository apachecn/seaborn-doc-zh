...
函数[`set_palette()`](../generated/seaborn.set_palette.html#seaborn.set_palette "seaborn.set_palette")接受相同的参数，并将为所有点设置默认的色彩循环。您也可以在`with`语句中调用[`color_palette()`](../generated/seaborn.color_palette.html#seaborn.color_palette "seaborn.color_palette")来临时改变调色版。([参见](#palette-contexts))
...
在不了解数据特征的情况下，通常不可能知道哪种调色板或色彩映射最适合一组数据。接下来，我们将通过三种常见的调色板 _定性调色板_, _顺序调色板_,_发散调色板_ 来拆分介绍使用[`color_palette()`](../generated/seaborn.color_palette.html#seaborn.color_palette "seaborn.color_palette")的不同方法以及其他seaborn函数。

## 定性调色板

当您想要区分不具有内在顺序的离散数据块时，定性(类别)调色板是最佳方案。

导入seborn时，默认色彩循环将更改为一组六种颜色并将唤起标准matplotlib色彩循环，同时使您看起来更赏心悦目。
...
当您要区分任意数量的类别而不强调任何类别时，最简单的方法是在圆形颜色空间中绘制间距相等的颜色(在此颜色空间中，色调会发生变化，同时保持亮度和饱和度不变)。这是大多数seaborn函数在处理所需颜色多于默认色彩循环中的颜色时所使用的默认方法。

最常见的方法是使用`hls‘颜色空间——一种简单的RGB值变体。
